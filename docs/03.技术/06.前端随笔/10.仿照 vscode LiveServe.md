...
class LRUCache {
  ...
  set(key, value) {
    if (this.cache.has(key)) {
      // 如果键已存在，则先删除该键，在后续重新插入 Map 末尾
      this.cache.delete(key);
    } else if (this.cache.size >= this.capacity) {
      // 如果缓存已满，则删除最久未使用的键（即 Map 中第一个键）
      const oldestKey = this.cache.keys().next().value;
      this.cache.delete(oldestKey);
    }
    this.cache.set(key, value); // 插入新的键值对到 Map 末尾
  }
...
const refreshPage = (path) => {
  const req = http.request({
    ...
    port: port,
    path: path,
    method: 'GET' // 使用 GET 请求刷新页面
  }, (res) => {
  });

  req.on('error', (err) => {
    console.error(`请求遇到错误：${err.message}`);
  });
