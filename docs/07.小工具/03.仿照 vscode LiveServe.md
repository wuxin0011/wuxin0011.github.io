...
<head>
  ...
  * {
  ...
  html,body {
    height: 100vh;
    background:url("https://cdn.jsdelivr.net/gh/wuxin0011/blog-resource@main/fantasy/screen.png");
...
const server = http.createServer((request, response) => {
  ...
  if (!(req_url.startsWith('https://') || req_url.startsWith('http://'))) {
    console.log(req_url)
...
const responseContent = (request, response) => {
  ...
  try {
    ...
    if (status.isDirectory()) {
      console.log('is dir')
      curReadFolder(request, response, real_url)
    } else {
      const content = readFile(real_url)
      if (!content) {
        responseErrorPage(request, response, "请求内容不存在")
      } else {
        responseTemplate(request, response, new Page(requestUrl, content, false))
      }
    }
  } catch (error) {
 ...
 * @param {*} folderPath 路径
 */
const curReadFolder = (request, response, folderPath = __dirname) => {
  let temp = ''
  ...
  responseTemplate(request, response, content ? new Page(reqUrl, content, true) : NOT_FOUND_PAGE)
}
